...
use rusty::config::{
    ConfigLoader, SurveyConfig, OverviewConfig, ModelConfig, IoConfig, 
    ProcessingConfig, OutputConfig, QualityConfig, RuntimeConfig, DagsConfig,
    load_survey_config
};
use rusty::config::model::{
    DagDefinition, TaskDefinition, RetryConfig, BackoffStrategy, TaskDefaults
};
use rusty::processing::DagExecutor;
use rusty::error::{Result, ConfigError};
...
"#,
    ...
    write_yaml_file(
        surveys_dir.join("TEST").join("overrides").join("env").join("prod.yml"),
        r#"
processing:
  max_threads: 8
  parallel: true
...
"#,
    ...
    assert_eq!(config.processing.as_ref().unwrap().strategy.to_string(), "chunked");
    assert_eq!(config.processing.as_ref().unwrap().memory_limit, Some(2000000));
...
"#,
    ...
    assert_eq!(config.processing.as_ref().unwrap().strategy.to_string(), "in_memory");
...
"#,
    ...
    write_yaml_file(
        surveys_dir.join("TEST").join("overrides").join("defaults.yml"),
        r#"
overview:
...
"#,
    ...
    write_yaml_file(
        surveys_dir.join("TEST").join("overrides").join("env").join("dev.yml"),
        r#"
overview:
...
"#,
    ...
    assert_eq!(config.overview.as_ref().unwrap().name, "Survey from dev environment");
    assert_eq!(config.overview.as_ref().unwrap().description, Some("Description from overview.yml".to_string()));
...
fn test_macro_resolution() -> Result<()> {
    ...
    // Create a macro file
    write_yaml_file(
        surveys_dir.join("_shared").join("macros").join("common_processing.yml"),
        r#"
standard_processing:
  strategy: chunked
  max_threads: 4
