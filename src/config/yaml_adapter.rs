...
use crate::config::model::{
    BackoffStrategy, ChunkingConfig, DagDefinition, DagsConfig, DataModel, FieldDefinition,
    FileFormatConfig, IndexDefinition, InputConfig, InputValidationConfig, IoConfig,
    IoOutputConfig, MmapConfig, ModelConfig, NamingConfig, ProcessingConfig, ProcessingStrategy,
    RetryConfig, SlaConfig, TaskDefinition,
};
    ...
    pub fn to_model_config(&self) -> ModelConfig {
        ...
        if let Some(series) = &self.series {
            let fields = series
                .schema
                .iter()
                .map(|(name, schema)| FieldDefinition {
                    name: name.clone(),
        ...
        if let Some(lookups) = &self.lookups {
            for lookup_def in lookups {
                ...
                let model = DataModel {
                    ...
                    description: lookup_def.description.clone(),
                    fields,
...
pub fn load_yaml_model_config(path: &PathBuf) -> Result<ModelConfig> {
    ...
    let yaml_config: YamlModelConfig = serde_yaml::from_str(&content).map_err(|e| {
    ...
    pub fn to_io_config(&self) -> IoConfig {
        let (base_dir, patterns) = if let Some(discovery) = &self.discovery {
            ...
            for pattern in &discovery.include {
                ...
                } else if pattern.contains(".area")
                    || pattern.contains(".item")
                    || pattern.contains(".footnote")
                    || pattern.contains(".period")
                    || pattern.contains(".contacts")
                    || pattern.contains(".seasonal")
                {
            ...
            // Fallback defaults
            (
                PathBuf::from("data/raw/bls"),
                vec!["*.series".to_string(), "*.data.*".to_string()],
            )
        };
...
pub fn load_yaml_io_config(path: &PathBuf) -> Result<IoConfig> {
    ...
    let yaml_config: YamlIoConfig = serde_yaml::from_str(&content).map_err(|e| {
        Error::Config(ConfigError::ParseError {
...
pub struct YamlProcessingConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub strategy: Option<YamlProcessingStrategy>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub resources: Option<YamlProcessingResources>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub io: Option<YamlProcessingIo>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub features: Option<YamlProcessingFeatures>,
}
...
impl YamlProcessingConfig {
    pub fn to_processing_config(&self) -> ProcessingConfig {
        ...
        if let Some(resources) = &self.resources {
            if let Some(t) = resources.max_threads {
                cfg.max_threads = t;
            }
        ...
        if let Some(features) = &self.features {
            if let Some(use_mmap) = features.use_mmap {
                cfg.mmap.enabled = use_mmap;
            }
        }
        cfg
    }
...
pub fn load_yaml_processing_config(path: &PathBuf) -> Result<ProcessingConfig> {
    ...
    let yaml_config: YamlProcessingConfig = serde_yaml::from_str(&content).map_err(|e| {
        Error::Config(ConfigError::ParseError {
            message: format!("Failed to parse YAML processing config: {}", e),
            line: None,
            column: None,
        })
    })?;
...
pub struct YamlDagsConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub settings: Option<YamlDagSettings>,
}
...
pub struct YamlDagGraph {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub edges: Option<Vec<YamlDagEdge>>,
}
...
pub struct YamlDagTask {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub retries: Option<YamlDagRetries>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct YamlDagRetries {
    pub max: Option<u32>,
    pub backoff_sec: Option<u64>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct YamlDagEdge {
    pub from: String,
    pub to: String,
}
    ...
    pub fn to_dags_config(&self) -> DagsConfig {
        ...
        for t in &self.graph.tasks {
            ...
            if let Some(r) = &t.retries {
                if let Some(m) = r.max {
                    retry.max_attempts = m;
                }
                if let Some(d) = r.backoff_sec {
                    retry.delay = d;
                }
            }
            let sla = t.sla_minutes.map(|m| SlaConfig {
                max_duration: m,
...
pub fn load_yaml_dags_config(path: &PathBuf) -> Result<DagsConfig> {
    ...
    let yaml_config: YamlDagsConfig = serde_yaml::from_str(&content).map_err(|e| {
        Error::Config(ConfigError::ParseError {
            message: format!("Failed to parse YAML dags config: {}", e),
            line: None,
            column: None,
        })
    })?;
